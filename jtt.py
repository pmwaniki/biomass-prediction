...
train_pred=joblib.load(os.path.join(result_dir,f'Train-predictions-Regression-SGD.joblib'))

Train['pred']=train_pred
Train['error']= np.abs(Train['biomass']-Train['pred'])

sns.scatterplot(x='pred',y='biomass',data=Train.sample(1000))
plt.show()


class H5DatasetWeighted(H5Dataset):
    def __init__(self,data,h5_path,return_outcome=False,error_col='error'):
        super().__init__(data,h5_path,return_outcome)
        self.error_col=error_col
    def __getitem__(self, idx):
        weight = self.data.iloc[idx][self.error_col]
...
configs = {
    ...
    # 'jtt_weight':tune.loguniform(0.0001,1.0)
...
def get_train_loader(config,train_data):
    train_dataset = H5DatasetWeighted(train_data,train_h5path,return_outcome=True)
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=config['batch_size'],num_workers=5)
    return train_loader

def get_val_loader(val_data):
    val_dataset = H5DatasetWeighted(val_data,train_h5path,return_outcome=True)
...
def train_fun(model,optimizer,criterion,train_loader,val_loader,scheduler=None,clip_norm=1.0,aug=None):
    ...
    for batch_x,batch_y,batch_weight in train_loader:
        ...
        batch_weight = batch_weight.to(device,dtype=torch.float).unsqueeze(1)
        if aug is not None:
            batch_x=aug(batch_x)
        # batch_x=normalize(batch_x)
        # batch_x = batch_x - 0.5
        pred=model(batch_x)
        loss=criterion(pred,batch_y,batch_weight)
        optimizer.zero_grad()
        loss.backward()
    ...
    obs_val = []
    with torch.no_grad():
        for batch_x, batch_y,batch_weight in val_loader:
            batch_x, batch_y = batch_x.to(device, dtype=torch.float), batch_y.to(device,dtype=torch.float).unsqueeze(1)
            batch_weight = batch_weight.to(device, dtype=torch.float).unsqueeze(1)
            # batch_x=normalize(batch_x)
            # batch_x=batch_x - 0.5
...
for c in clusters:
    ...
    train_data['pred'] = scl_biomass.transform(train_data['pred'].values.reshape(-1, 1)).reshape(-1)
    val_data['biomass'] = scl_biomass.transform(val_data['biomass'].values.reshape(-1, 1)).reshape(-1)
    val_data['pred'] = scl_biomass.transform(val_data['pred'].values.reshape(-1, 1)).reshape(-1)
    train_data['error']=np.abs(train_data['biomass']-train_data['pred'])
    ...
    metric='loss';mode="min"; scope='last'
