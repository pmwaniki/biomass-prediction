...
configs = {
    ...
    'hidden_dim': tune.choice([4,8,16,32,64]),
...
class CNNBayesian(pyro.nn.PyroModule):
    ...
    def forward(self, x, y=None):
        x = self.cnn(x)
        x_mean = x[:, 0]
        x_var = F.softplus(x[:, 1])+1e-5
        mean = pyro.deterministic("mean", x_mean)
        with pyro.plate("data", x.shape[0]):
            obs = pyro.sample("obs", dist.Normal(mean, x_var), obs=y)
        return mean
def get_model(config):
    base_model=CNN(dropout=config['dropout'],dim_out=2,hidden_dim=config['hidden_dim'],
    ...
    model=CNNBayesian(base_model)
    if config['full_bayes']:
                ...
                for name, value in list(m.named_parameters(recurse=False)):
                    if name in ('weight', 'bias'):
                        setattr(m, name, PyroSample(
                            prior=dist.Normal(torch.tensor(0.0, device=device), torch.tensor(0.1, device=device))
                            .expand(value.shape)
                            .to_event(value.dim())))
    return model
...
def train_fun(model,guide,optimizer,train_loader,val_loader,scheduler=None,clip_norm=1.0,aug=None):
    model.train()
    train_loss=0
    for batch_x,batch_y in train_loader:
        ...
        loss=pyro.infer.Trace_ELBO().differentiable_loss(model,guide,batch_x,batch_y)
        optimizer.zero_grad()
        loss.backward()
    ...
    with torch.no_grad():
        for batch_x, batch_y in val_loader:
            ...
            pred_val.append(pred.cpu().numpy().reshape(-1))
            obs_val.append(batch_y.cpu().numpy().reshape(-1))
...
for c in clusters:
    ...
    class Trainer(tune.Trainable):
        def setup(self, config):
            ...
            self.guide=null_guide
            self.optimizer=get_optimizer(config,self.model)
            # self.criterion=pyro.infer.Trace_ELBO().differentiable_loss()
        ...
        def step(self):
            train_loss,loss,r2,mse=train_fun(self.model,self.guide,self.optimizer,self.train_loader,self.val_loader,
    ...
    # Test model accuracy
    best_guide=null_guide
    n_samples = 1000
    predictive = Predictive(best_model, guide=best_guide, num_samples=n_samples,
                            return_sites=("obs",))
    ...
    with torch.no_grad():
        for batch_x in test_loader:
            ...
            pred = predictive(batch_x)['obs']
            pred_test.append(pred.t().cpu().numpy())

    pred_test = np.concatenate(pred_test,axis=0)
    for i in range(pred_test.shape[1]):
        pred_test[:,i]=scl_biomass.inverse_transform(pred_test[:,i].reshape(-1,1)).reshape(-1)
    # pred_test = scl_biomass.inverse_transform(pred_test.reshape(-1,1)).reshape(-1)
    cluster_predictions.append(pred_test)


joblib.dump(cluster_predictions,filename=os.path.join(result_dir,f"Submission-samples-{experiment}.joblib"))
pred_test=np.concatenate(cluster_predictions,axis=1)
pred_test=pred_test.mean(axis=1) + 30.0
...
cluster_predictions2=[c.mean(axis=1) for c in cluster_predictions]
unweighted_pred=np.stack(cluster_predictions2).mean(axis=0)

preds_unweighted = pd.DataFrame({'ID':Submission["ID"],'Target':unweighted_pred})
#%%
preds_unweighted.to_csv(os.path.join(result_dir,f'Submission-{experiment}-unweighted.csv'), index=False)
